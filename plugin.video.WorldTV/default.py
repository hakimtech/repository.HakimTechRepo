...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.royalty/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.royalty/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to WorldTV" 
			line3 = ('[COLOR white]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('WorldTV', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
##########################################
def catchup():
    loginurl   = "http://royaltv.xyz:25461/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
        ...
        print e.getcode()
        dialog.ok("[COLOR yellow]Expired Account[/COLOR]",'[COLOR yellow]You cannot use this service with an expired account[/COLOR]',' ','[COLOR yellow]Please check your account information[/COLOR]')
        sys.exit(1)
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        try: 
            ...
            mbs = '[COLOR yellow]%.02f MB of less than 5MB[/COLOR]' % (currently_downloaded)
            e = '[COLOR yellow]Speed:  %.02f Mb/s ' % mbps_speed  + '[/COLOR]'
        ...
        if dp.iscanceled():
            ...
            dialog.ok("WorldTV", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search WorldTV:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('WorldTV', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('WorldTV', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('WorldTV', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('WorldTV', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('WorldTV', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.royalty/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('WorldTV', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.WorldTV')
	...
	loginurl   = "http://royaltv.xyz:25461/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://royaltv.xyz:25461/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('WorldTV','Would You like us to Setup the PVR Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select PVR To Setup', ['PVR TV Guide'])
			if dialog==0:
				pvrsetup()
				xbmcgui.Dialog().ok('WorldTV', 'PVR Integration Complete')
			elif dialog==1:

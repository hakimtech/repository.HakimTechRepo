...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('Force Close Hakim Technology', '[COLOR %s]You are about to close Hakim Technology' % COLOR2, 'Would you like to continue?[/COLOR]', nolabel='[B][COLOR red] No Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Force Close Hakim Technology[/COLOR][/B]')
	if choice == 1:
		log("Force Closing Hakim Technology: Platform[%s]" % str(platform()), xbmc.LOGNOTICE)
		os._exit(1)
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]WARNING: Sometimes Reloading the Profile causes Hakim Technology to crash.  While Hakim Technology is Reloading the Profile Please Do Not Press Any Buttons![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
